...
- **Test without hardware**
  ...
  - **Run & lint SCPI scripts.** `visa script foo.scpi` runs a `.scpi` file (SCPI commands + inline assertions, [ADR 0027](docs/adr/0027-phase3-operator-automation.md)) against the current device; `visa lint foo.scpi` flags unknown SCPI roots (IEEE 488.2 + SCPI core) before you run it.
