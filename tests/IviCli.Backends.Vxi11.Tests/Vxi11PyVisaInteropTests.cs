...
/// <summary>
/// Wire-level interop coverage: PyVISA (via pyvisa-py's '@py' VXI-11
/// transport) drives an in-proc <see cref="Vxi11GatewayServer"/> over
/// loopback and reads back the response that the gateway routes to
/// <see cref="FakeBackend"/>. Skipped on CI images without python +
/// pyvisa via the Requires gate from Batch C.
/// </summary>
public sealed class Vxi11PyVisaInteropTests
{
    ...
    public async Task PyVisa_can_idn_query_in_proc_vxi11_gateway()
    {
        ...
        var fake = new FakeBackend().RespondToQuery(deviceName, "*IDN?", "FAKE,VXI11,IVI-CLI,1.0");
        ...
        stdout.Trim().ShouldBe("FAKE,VXI11,IVI-CLI,1.0");
    }
